...
void readtree_roads::Loop(TString key, Int_t charge)
{
	...
	TH2F * n_stub_ss_lay = new TH2F ("n_stub_ss_lay","",11,0,11,512,0,512);
	...
	for(int l =0; l<11; ++l){
		sprintf(name,"nstubs_xy_id0_lay_%2i",l);
		xy[l] = new TH2F(TString(name),"xy_",500,-30,130,500,-10,130);
		sprintf(name,"nstubs_xy_road_w/_id0_per_lay_%2i",l);
...
//	};
	...
	nentries = 5000; //set #evt to check
		...
		for(int r =0; r<nroads; ++r){
			...
			for(int l=0; l<6;++l){
				...
				if(ssid == 0) {
					n_lay_w_ssid0[lay]++;
					unsigned int stub;
					for(int y=0;y<stubref[l].size();y++){
						stub=stubref[l][y];
						xy[lay]->Fill(TTStubs_r->at(stub)*cos(TTStubs_phi->at(stub)),TTStubs_r->at(stub)*sin(TTStubs_phi->at(stub)));
					}
				}
			}
		}
...
void readtree_roads_densitystubs33(TString name = "0", TString key = "0",Int_t charge=1){
	...
	if(key.Index("cmseos")!=-1) fName = "root://cmsxrootd.fnal.gov//store/user/gvidale/TT33/"+name+".root"; //for eos file in my home
